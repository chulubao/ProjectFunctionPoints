...
#define    RET_SUCCESS      0    /* Successful return status.*/
    ...
    bool processSingletonRun() {
        ...
        if (fd < 0) {
            if (errno ==
                EEXIST) {    //logdebug(LOG_INFO,LOG_DEBUG_PATH,"%s\t %d\t %s\t tips=%s\n",__FUNCTION__,__LINE__,__FILE__,"进程已经存在，并不要重复执行");
                return false;
            }
        }
    ...
    void recvByTcp() {
        string returnMsg;
        createXml(string("daemonAccount"), string("uiAccount"), string("Account"), string("Account"), \
        string("1"),string("0"),returnMsg);
        nsCommunication::replyMsg = returnMsg;
        while (1) {
            recvMsgSynSendMsgByTcp(LOCAL_SOCKET_PATH);
            std::cout << "\n=========================================\n";
            printf("[%s]\n", recvMsg);
            parseXml(string(recvMsg));
    ...
    void processOption(int argc, char *argv[]) {
        ...
        if (RET_FAILURE == ret) {
             ...
             *exit(x)（x不为0）都表示异常退出.
             *********************************/
            logdebug(LOG_ERR, LOG_PRINT_FILE, "EXIT_FAILURE");
            exit(EXIT_FAILURE);
        }
        return;
